...
sigma_points = MerweScaledSigmaPoints(n=8, alpha=0.1, beta=2, kappa=-5)  # kappa = 3-n (n = dim_x)

ukf = UKF(dim_x=8, dim_z=1, hx=hx_v, fx=fx, dt=0.02, points=sigma_points)
ukf.x = np.array([0, 0, 0, 0, 0, 0, 0, 0])

ukf.P = np.diag([0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001])  # Initial state covariance 
...
for index, row in sensor_data.iterrows():
    z = np.array([row['distance_44AE'], row['distance_06A5'], row['distance_CBB0'], row['distance_4F9B'], row['v_x'], row['a_x'], row['v_y'], row['a_y'], row['yaw_imu'], row['w']])
    t = row['time']
    dt = t - last_t
    last_t = t
    ...
    if flag==1:  # start predicting
        ...
        # Prediction step
        print("x prima della predizione:", ukf.x)
        print("P prima della predizione:", ukf.P)
        
        ukf.predict(dt=dt, fx=fx)

        print("x dopo la predizione:", ukf.x)
        print("P dopo la predizione:", ukf.P)
        ...
        if z[8]!=0 and z[5]!=0 and z[7]!=0:  # imu
            for i, (zi, hx_func, R_val) in enumerate(zip([z[8], z[5], z[7]], 
                                                          [hx_theta, hx_a_x, hx_a_y],
                                                          [0.00289, 0.96236, 0.96236])):
                ukf.R = np.array([[R_val]])
                ukf.update(zi, hx=hx_func)

    
        if z[4]!=0 or z[9]!=0 or (z[0]==0 and z[1]==0 and z[2]==0 and z[3]==0 and z[4]==0 and z[5]==0 and z[6]==0 and z[7]==0 and z[8]==0 and z[9]==0):  # odom
            for i, (zi, hx_func, R_val) in enumerate(zip([z[4], z[9]], 
                                                          [hx_v, hx_w],
                                                          [0.1, 0.01])):
                ukf.R = np.array([[R_val]])
                ukf.update(zi, hx=hx_func)
